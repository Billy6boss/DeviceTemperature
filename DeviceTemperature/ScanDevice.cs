...
namespace DeviceTemperature {
    class ScanDevice {
        ...
        public PCinfoModel getPcInfo () {
            ...
            try {
                if ( new ManagementObjectSearcher (@"SELECT * FROM Win32_Processor") ==null  || new ManagementObjectSearcher (@"SELECT * FROM Win32_Processor").Get().Count <= 0) {
                    thisPC.proesser = "No CPU";
                } else {
                ...
                if ( new ManagementObjectSearcher (@"SELECT * FROM Win32_BaseBoard") ==null  || new ManagementObjectSearcher (@"SELECT * FROM Win32_BaseBoard").Get ().Count <= 0) {
                    thisPC.motherBoard = "No motherBoard";
                } else {
                ...
                int i = 0;
                if ( new ManagementObjectSearcher (@"Select * From Win32_PhysicalMemory") ==null || new ManagementObjectSearcher (@"Select * From Win32_PhysicalMemory").Get ().Count <= 0) {
                    RamModel thisRam = new RamModel ();
                    thisRam.Maker = "No Ram";
                    thisPC.rams.Add(thisRam);
                } else{
                    foreach (var RAM in new ManagementObjectSearcher(@"Select * From Win32_PhysicalMemory").Get()) {

                        RamModel thisRam = new RamModel();
                        thisRam.Maker = Convert.ToString(RAM["Manufacturer"]);
                        thisRam.speed = Convert.ToString(RAM["Speed"]) + "MHz";
                ...
                if ( new ManagementObjectSearcher (@"Select * From Win32_DiskDrive") ==null  || new ManagementObjectSearcher (@"Select * From Win32_DiskDrive").Get ().Count <= 0) {
                    deviceModle thisdevice = new deviceModle ();
                    thisdevice.modleName = "No Drive";
                ...
                } else
                {
                    foreach (var Drive in new ManagementObjectSearcher (@"Select * From Win32_DiskDrive").Get ()) {
                        ...
                        thisdevice.modleName = Convert.ToString(Drive["Model"]) + ++i;

                        if (Convert.ToInt64(Drive["Size"]) > 0) { 
                ...
                if ( new ManagementObjectSearcher (@"Select * From Win32_VideoController") ==null || new ManagementObjectSearcher (@"Select * From Win32_VideoController").Get ().Count <= 0) {

                    thisPC.videoCard = "No GPU";
                } else{
                    foreach (var GPU in new ManagementObjectSearcher(@"Select * From Win32_VideoController").Get()) {

                        thisPC.videoCard = Convert.ToString(GPU["Name"]);
                    }
                }

                if ( new ManagementObjectSearcher(@"Select * From Win32_OperatingSystem").Get() ==null  || new ManagementObjectSearcher(@"Select * From Win32_OperatingSystem").Get().Count <= 0) {
                    thisPC.os = "No OS";
                } else {
                    foreach (var OS in new ManagementObjectSearcher(@"Select * From Win32_OperatingSystem").Get()) {
                        ...
                        switch (Convert.ToString(OS["BuildNumber"])) {
                            case "2600":
                                version = "Windows XP";
                                break;
                            case "2600.1105-1106":
                ...
                string debugTxetPath = "C:\\getPcInfo.txt";
                using (TextWriterTraceListener _DebugLog = new TextWriterTraceListener(debugTxetPath)) {
                    Trace.Listeners.Add(_DebugLog);
                    Trace.AutoFlush = true;
                    Trace.WriteLine("SQL Area error:" + ex.StackTrace.ToString() + ex.Source.ToString() + ex.Data.ToString());
                    Trace.WriteLine(ex.ToString());
                    Trace.Write(thisPC.proesser +"\n"+ thisPC.motherBoard + "\n" + thisPC.rams.ToArray() + "\n" + thisPC.drives.ToArray() + "\n"  + thisPC.videoCard + "\n" + thisPC.os);
